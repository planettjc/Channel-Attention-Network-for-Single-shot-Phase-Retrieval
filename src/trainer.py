...
class Trainer():
    ...
    def train(self):
        ...
        for batch, (inp, ph, _,) in enumerate(self.loader_train):
            inp, ph = self.prepare(inp, ph)
            timer_data.hold()
            timer_model.tic()

            self.optimizer.zero_grad()
            oup = self.model(inp)
            loss = self.loss(oup, ph)
            loss.backward()
    ...
    def test(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            for inp, ph, filename in tqdm(d, ncols=80):
                inp, ph = self.prepare(inp, ph)
                oup = self.model(inp)
                oup = utility.quantize(oup, self.args.inten_range)

                save_list = [oup]
                self.ckp.log[-1, idx_data, 0] += utility.calc_psnr(
                    oup, ph, self.args.inten_range, dataset=d
                )
                if self.args.save_gt:
                    save_list.extend([inp, ph])
